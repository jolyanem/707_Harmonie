...
const step7_1Schema = z.object({
  status: z.string(),
});

const Step7_1 = ({ ursId, step, readonly, setReadonly }: StepProps) => {
  const router = useRouter();
  const updateStepMutation = useMutation({
    mutationKey: ['updateStep', ursId, '7_1'],
    mutationFn: (values: z.infer<typeof step7_1Schema>) =>
      axios.put<URSDto>(`/urs/${ursId}/step/7_1`, values),
    onSuccess: () => {
  ...
  const form = useForm<z.infer<typeof step7_1Schema>>({
    resolver: zodResolver(step7_1Schema),
    defaultValues: step,
    disabled: readonly,
  });
  ...
  function onSubmit(values: z.infer<typeof step7_1Schema>) {
  ...
  return (
    <div>
      <h2 className="font-semibold text-lg uppercase">
        ÉTAPE 7.1 | TESTS INTÉGRATION (AMOCIS) | PRÉ-RECETTE
      </h2>
      <Form {...form}>
        <form
