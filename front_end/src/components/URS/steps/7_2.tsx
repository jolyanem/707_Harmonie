...
const step7_2Schema = z.object({
  status: z.string(),
});

const Step7_2 = ({ ursId, step, readonly, setReadonly }: StepProps) => {
  const router = useRouter();
  const updateStepMutation = useMutation({
    mutationKey: ['updateStep', ursId, '7_2'],
    mutationFn: (values: z.infer<typeof step7_2Schema>) =>
      axios.put<URSDto>(`/urs/${ursId}/step/7_2`, values),
    onSuccess: () => {
      toast.success('Étape 7.1 mise à jour avec succès');
      setReadonly(true);
      router.invalidate();
    },
  });

  const form = useForm<z.infer<typeof step7_2Schema>>({
    resolver: zodResolver(step7_2Schema),
    defaultValues: step,
    disabled: readonly,
  });
  ...
  function onSubmit(values: z.infer<typeof step7_2Schema>) {
  ...
  return (
    <div>
      <h2 className="font-semibold text-lg uppercase">
        ÉTAPE 7.2 | TESTS utilisateurs (AMOCIS / client) | RECETTEs
      </h2>
      <Form {...form}>
        <form
